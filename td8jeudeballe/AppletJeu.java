...
private static Button modeNormal;
private static Button modeScore;
...
private static Button temps;
private int type; //Type de difficulté actuelle
private boolean isblockTemps; //Détermine si le niveau contre la montre est bloqué ou pas
	...
	public void init(){
		...
		p2.setLayout(new BorderLayout()); p3.setLayout(new FlowLayout()); p4.setLayout(new FlowLayout()); p5.setLayout(new BorderLayout());
		add(p2, BorderLayout.WEST);
		p2.setVisible (true);
		p2.add(p3, BorderLayout.NORTH); p2.add(p4, BorderLayout.SOUTH);
		p3.setVisible (true); p4.setVisible (true); p5.setVisible (true);
		//Bouton du choix de mode de jeu
		mode = new Label("Mode : Normal", Label.CENTER);
		modeNormal = new Button("Normal");
		modeScore = new Button("Score");
		p1.add(mode); p1.add(modeNormal); p1.add(modeScore); 
		//Boutons de difficulté
		...
		p1.add(difficulte); p1.add(tresFacile); p1.add(facile); p1.add(normal); p1.add(hard); p1.add(legendaire); p1.add(temps); 
		//Bouton pour recommancer
		...
		modeNormal.addActionListener(this);
		...
		//Cheat minable pour avoir des boutons pas trop gros
	...
	public void newBoutons(){
		if(modeActuel){ //Si c'est le mode normal
			p1.removeAll(); 
			p1.add(mode); p1.add(modeNormal); p1.add(modeScore);
			p1.add(difficulte); p1.add(tresFacile); p1.add(facile); p1.add(normal); p1.add(hard); p1.add(legendaire); p1.add(temps);
			p1.add(new Label(" ", Label.CENTER)); 
			...
			p1.removeAll();
			p1.add(mode); p1.add(modeNormal); p1.add(modeScore); 
			p1.add(difficulte); p1.add(facile); p1.add(normal); p1.add(hard); p1.add(legendaire); p1.add(temps);
			p1.add(new Label(" ", Label.CENTER));
			p1.add(new Label(" ", Label.CENTER)); 
			p1.add(new Label(" ", Label.CENTER));
		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == recommencer){
		...
		else if (e.getSource() == modeNormal){
			mode.setText("Mode : Normal");
			modeActuel = true;
			//On remet la difficulté à facile par défaut
			type = 1; difficulte.setText("Difficulté : Facile");
			newDamier();
			...
			mode.setText("Mode : Sore");
			modeActuel = false;
			//On remet la difficulté à facile par défaut
			type = 1; difficulte.setText("Difficulté : Facile");
			newDamier(); 
			regle.setText("Déplacez la balle verte pour absorber les pierres précieuses. Ces rubis sont d'une grande rareté.'");
			...
			if(isblockTemps){
				...
				if(inputWord.equals("toast")) {
					...
					difficulte.setText("Contre la montre");
					temps.setLabel("Contre la montre");
					type = 5;
					newDamier();
					regle.setText("Déplacez la balle verte vers la case bleue d'arrivée, évitez à tout prix les cases jaunes meurtrières !");
					regle2.setText("Certaines cases hostiles se développent de manière anormale, vous avez un temps limité pour espérer agir.");
				}
				else{
