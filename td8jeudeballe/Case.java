...
public class Case{
	...
	private boolean enVoieDeDisparition; //Si la case est absorbée
...
/*********************** Bouger : mouvements horizontaux/verticaux à partir de facile ***********************/	
	...
	public void bougerXY(){
		...
		if(XY){ // En cas de mouvement horizontal
			...
			if ((orig.abscisse()<(coteCase+1) && orig.ordonne()<=1) || (orig.abscisse()>coteCase*(nbCase-2)+1 && orig.ordonne()>coteCase*(nbCase-1))){
				if(XX) XX = false;
				else XX = true;
			}
			...
			//Pour éviter les cases de départ et d'arrivée
			if ((orig.abscisse()<=1 && orig.ordonne()<(coteCase+1)) || (orig.abscisse()>coteCase*(nbCase-1) && orig.ordonne()>coteCase*(nbCase-2)+1)){
				if(XX) XX = false;
				else XX = true;
			}
			// Régler le sens du mouvement
...
/*********************** Bouger : mouvement oblique ajouté à partir de difficile **************************/	
	...
	public void bougerXYZ(){
		...
		if (XYZ){
			//Pour éviter les cases de départ et d'arrivée
			if(orig.ordonne()<=1 && orig.abscisse()<=coteCase+2){
				bougerOblique(0);
				XXZ = 0;
			}
			if(orig.ordonne()<=coteCase+2 && orig.abscisse()<=1){
				bougerOblique(0);
				XXZ = 0;
			}
...
/************************************ dessiner **********************************************/	
	...
	public void dessiner(Graphics g){
		g.setColor(fond);
		//On fait appel à la disparition si la case est absorbée
		if(enVoieDeDisparition){
			disparition();
		}
